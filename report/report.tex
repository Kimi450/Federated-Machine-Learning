...
In this section, we introduce several extensions to Google's federated learning idea. These extensions are based on different averaging methodologies in a central and peer-to-peer averaging context. We will also look at the case where the users do not share their weights at all in the localised training idea.
\subsection{Central}
...
\end{equation}
In the case where the choice of metric is such that a lower value is better than a higher one, such as in loss, we replace $E_i$ by its inverse $\frac{1}{E_i}$ in equation~\ref{eq:weighted-avg}.
\\\\
The design of the federated learning process with weighted averaging is very similar to what is described in Section~\ref{subsec:fedml-core} with the only a couple of changes. The first change is that the user must also share their latest evaluations along with their weights. We change algorithms~\ref{algo:fedcentral_user} and~\ref{algo:fedcentral} to algorithms~\ref{algo:fedcentral-weightedavg_user} and~\ref{algo:fedcentral-weightedavg} respectively to reflect this change.
\begin{center}\begin{algorithm}[H]
\texttt{\Fn{\FFedCoreUser{new\_weights}}{
	\If{new\_weights != None}{
		$M_i$.set\_weights(new\_weights)\\	
...
\end{algorithm}\end{center}
\noindent Algorithm~\ref{algo:fedp2p} starts off with sending all the users in $U$ the model $M$ that they will be using in this process. Then, for every round of federated learning, we start off by training all the users which is illustrated in algorithm~\ref{algo:fedp2p_user}. In this process, if a set of weights is provided to a user $U_i$, it will calculate the averaged weights based on the local averaging methodology and then initialise the model with the newly computed averaged weights. The averaging will be discussed more in depth in the following section. If no weights are provided to the user, the model weights are not changed. The model is then evaluated on the test data and the evaluation data is stored in the $pre\_fit$ evaluations. After doing so, the model is trained for a number of epochs and then once again evaluated on the test data and the evaluation data is stored in the $post\_fit$ evaluations. After the training process, all the weights from the users are collected and sent to every user in the next round of federated learning. This step is done explicitly like so as it is easier to show what exactly happens. In a real world scenario, this process would be done on the users end with the user just broadcasting their weights to every other user. But our design here satisfies the needs for simulating the idea. Then the next round of federated process begins and the whole process repeats for a set number of federated learning rounds.
\\\\

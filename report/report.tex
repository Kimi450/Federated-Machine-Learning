...
With the data and the model ready, we run the experiments and record the results as explained in Section~\ref{subsec:framework}. The experiments were ran where 64 federated rounds of learning took place with the local epoch count being 16. For the global user, the experiment ran for $64 \times 16$ epochs. The 
\\\\
\begin{figure}[H]
\centering
...
\end{subfigure}
\caption[short]{Graphs for the experiments on the postures dataset.}
...
\hline\end{tabular}}
\caption{Image dataset with $P$ = 12.50\%} \label{tab:sometab}
...
\hline
\end{tabular}}
\caption{Image dataset with $P$ = 70\%} \label{tab:sometab}

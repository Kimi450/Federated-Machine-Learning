...
\end{equation}
In \texttt{numpy\_split}, where we deal directly with numpy arrays, for the splitting we use the \texttt{split} function provided by numpy to split an array based on the given ratio. In \texttt{split\_dataframe}, where the splitting is done on DataFrames, we use scikit-learn's \texttt{train\_test\_split} function to split the DataFrame based on the given ratio. To ensure reproducibility, we use a \texttt{seed} for every randomised operation used in this project. The \texttt{train\_test\_split} function uses this as it shuffles the input DataFrame before splitting it. 
\\\\
As we are splitting the data in a stratified manner, we need to split the data based on the user. The parameter \texttt{for\_user} parameter in the splitting methods is used to filter out and split the data pertaining to only a specific user in the dataset. This is once again specific to the dataset and will be discussed in later sections that focus on a dataset. But the idea is that all the splitting functions return numpy arrays that represent the training, testing and validation data for a specific user. 
...
specific to dataset
read in the dataset
this data is split into 3, talk about them
store data from dataset for a user in the user object
user object also has model
...
seeding
talk about metric, sparse categorical
...
df split
model selection
In the case of a csv file, we can use the \texttt{read_csv} method that pandas provides. In the case of traversing a directory structure, we 
